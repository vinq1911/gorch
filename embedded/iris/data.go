...
// Fisher's Iris dataset, public domain. Each row is
//   {sepal_length, sepal_width, petal_length, petal_width, class}
// in centimeters, with classes 0=setosa, 1=versicolor, 2=virginica.
//
// We use a balanced subset (30 samples × 3 classes = 90 samples) of the
// classic 150-sample dataset. This is plenty for a 4-8-3 MLP to learn
// the decision boundary (expected >90% test accuracy), and keeps the
// embedded Go file small.
...
// Split splits Data into train and test sets at the given fraction
// (deterministic, stratified by class). Returns (Xtrain, Ytrain, Xtest, Ytest).
func Split(testFrac float32) (xTr, xTe [][]float32, yTr, yTe []int) {
	// Three classes; take the last testFrac of each class into the test set.
	byClass := make(map[int][][]float32)
	for _, row := range Data {
		c := int(row[4])
		byClass[c] = append(byClass[c], []float32{row[0], row[1], row[2], row[3]})
	}
	for c := 0; c < 3; c++ {
		rows := byClass[c]
		nTest := int(float32(len(rows)) * testFrac)
		nTrain := len(rows) - nTest
