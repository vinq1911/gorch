...
func main() {
	...
	flag.BoolVar(&memTrace, "mem-trace", false, "print a memory line per accumulation micro-step (footprint / Go heap / MTLBuffer bytes / cached MPSGraphs)")
	flag.BoolVar(&memTraceRegions, "mem-trace-regions", false, "with -mem-trace, also dump vmmap's per-region-type footprint breakdown each micro-step")
	flag.BoolVar(&unsafeNoPurge, "unsafe-no-purge", false, "DIAGNOSTIC ONLY: do not discard a released Metal buffer's pages. Reproduces footprint growing by cumulative allocation volume. Never use for a real run")
